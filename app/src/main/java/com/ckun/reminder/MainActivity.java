...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        if (state != null) {
            ...
            if (state.getBoolean("editing")) {
                ...
                durationInput.setText(state.getString("duration", "")); return;
            }
        }
    ...
    @Override protected void onSaveInstanceState(Bundle out) {
        ...
        if (editing) {
            ...
            out.putString("duration", durationInput.getText().toString()); out.putBoolean("ten", ten.isChecked()); out.putBoolean("day", day.isChecked());
        }
    }
    ...
    private void showEditor(Task t) {
        ...
        ten = new CheckBox(this); day = new CheckBox(this); ten.setTextColor(INK); day.setTextColor(INK);
        ten.setChecked(t.earlyTen); day.setChecked(t.earlyDay); page.addView(ten); page.addView(day); gap(14);
        label("预计持续时间 · 选填");
    ...
    private void updateTime() {
        ...
        updateOption(ten, "提前 10 分钟提醒", draft.start - ReminderRules.OFFSETS[1]);
        updateOption(day, "提前 1 天提醒（24 小时）", draft.start - ReminderRules.OFFSETS[2]); updateEnd();
    }
    private void updateOption(CheckBox option, String label, long at) {
        boolean valid = at > System.currentTimeMillis(); option.setEnabled(valid);
        option.setTextColor(valid ? INK : MUTED);
        if (!valid) option.setChecked(false);
        option.setText(valid ? label : getString(R.string.expired_reminder, label));
    }
    ...
    private void pickTime() {
        Calendar c = Calendar.getInstance(); c.setTimeInMillis(draft.start);
        new DatePickerDialog(this, (view, year, month, date) -> {
            ...
            new TimePickerDialog(this, (clock, hour, minute) -> {
                ...
                draft.start = c.getTimeInMillis(); updateTime();
    ...
    private void save() {
        ...
        updateTime(); draft.earlyTen = ten.isChecked(); draft.earlyDay = day.isChecked();
        store.save(draft); scheduler.schedule(draft); boolean needsPermission = !draft.done && (!scheduler.notificationsAllowed() || !scheduler.exactAllowed());
