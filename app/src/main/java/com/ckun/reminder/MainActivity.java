...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        if (state != null) {
            ...
            if (state.getBoolean("editing")) {
                ...
                t.sourceGoalId = state.getLong("sourceGoalId"); t.sourceCourseId = state.getLong("sourceCourseId"); t.generatedDay = state.getLong("generatedDay");
    ...
    @Override protected void onSaveInstanceState(Bundle out) {
        ...
        if (editing) {
            ...
            out.putLong("revision", draft.revision); out.putLong("completedAt", draft.completedAt); out.putLong("sourceGoalId", draft.sourceGoalId); out.putLong("sourceCourseId", draft.sourceCourseId); out.putLong("generatedDay", draft.generatedDay); out.putBoolean("done", draft.done);
    ...
    private void showList() {
        ...
        dateLabel = text(today(), 13, MUTED); dateLabel.setTag("home-date"); dateLabel.setSingleLine(true); dateLabel.setMinWidth(dp(54)); top.addView(dateLabel, new LinearLayout.LayoutParams(0, -2, 1));
        Button goals = button("目标", false, () -> startActivity(new Intent(this, GoalsActivity.class)));
        goals.setTag("goals"); compactHomeButton(goals);
        LinearLayout.LayoutParams goalParams = new LinearLayout.LayoutParams(-2, dp(30)); goalParams.rightMargin = dp(4); top.addView(goals, goalParams);
        ...
        LinearLayout.LayoutParams timetableParams = new LinearLayout.LayoutParams(-2, dp(30)); timetableParams.rightMargin = dp(4); top.addView(timetable, timetableParams);
        ...
        top.addView(stats, new LinearLayout.LayoutParams(-2, dp(30))); page.addView(top); gap(10);
    ...
    private void compactHomeButton(Button button) { button.setTextSize(11); button.setMinHeight(0); button.setMinWidth(0); button.setPadding(dp(8), dp(2), dp(8), dp(2)); }
